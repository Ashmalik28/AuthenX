...
const StatsCard = ({svg , metrics , headline , subheadline , classprops }) => {
    return <span className={`p-7 flex w-64 flex-col rounded-2xl outline-1 outline-gray-200 hover:scale-105 transition-all duration-300 ease-in-out items-center bg-white`}>
     ...
     <div className="mt-2 text-3xl font-medium">{metrics}</div>
     <div>{headline}</div>
     <div>{subheadline}</div>
    </span>
}
const statsData = [
    {svg : <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-12">
    ...
    {svg : <IoShieldCheckmark size={48}/> , metrics : "99.9%" , headline : "System" , subheadline : "Reliability" , classprops : "text-purple-600"},
    {svg: <GoOrganization size={48} /> , metrics : "50+" , headline : "Organizations" , subheadline : "Onboarded" , classprops : "text-amber-500" }
];
const Stats = () => {
    return (
        <div className="w-100vw mx-3 my-3 flex flex-col justify-center items-center rounded-2xl bg-gray-100 mt-20 h-72">
            ...
            <div className="flex items-center pl-20 pr-20 mt-5 gap-5">
